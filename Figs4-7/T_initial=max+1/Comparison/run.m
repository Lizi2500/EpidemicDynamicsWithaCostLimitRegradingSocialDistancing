...
T01=find(index_set_0{1}(:,2)<1,1,'first')-1;

index1=zeros(numel(alpha),1);
index1(T_index+sign(1-alpha_index)+T01<=T1)=1;  % there is no enough source
...
C_total_R_1=C_R.*R1;
C_total_V_1=C_V.*V1;
C_total_D_1=C_D.*(alpha'-1).^2.*min(T_index,T1-T01)+C_D.*(alpha_index-1).^2.*sign(max(T1-T01-T_index,0));
...
T02=find(index_set_0{1}(:,2)<1,1,'first')-1;

index2=zeros(numel(alpha),1);
index2(T_index+sign(1-alpha_index)+T02<=T2)=1;
index2(end)=0;
...
C_total_R_2=C_R.*R2;
C_total_V_2=C_V.*V2;
C_total_D_2=C_D.*(alpha'-1).^2.*min(T_index,T2-T02)+C_D.*(alpha_index-1).^2.*sign(max(T2-T02-T_index,0));
...
T03=find(index_set_0{1}(:,2)<1,1,'first')-1;

index3=zeros(numel(alpha),1);
index3(T_index+sign(1-alpha_index)+T03<=T3)=1;  % there is no enough source
...
C_total_R_3=C_R.*R3;
C_total_V_3=C_V.*V3;
C_total_D_3=C_D.*(alpha'-1).^2.*min(T_index,T3-T03)+C_D.*(alpha_index-1).^2.*sign(max(T3-T03-T_index,0));
...
T04=find(index_set_0{1}(:,2)<1,1,'first')-1;

index4=zeros(numel(alpha),1);
index4(T_index+sign(1-alpha_index)+T04<=T4)=1;
index4(end)=0;
...
C_total_R_4=C_R.*R4;
C_total_V_4=C_V.*V4;
C_total_D_4=C_D.*(alpha'-1).^2.*min(T_index,T4-T04)+C_D.*(alpha_index-1).^2.*sign(max(T4-T04-T_index,0));
